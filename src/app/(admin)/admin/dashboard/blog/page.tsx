...
import { sanityClient, BLOG_POSTS_ADMIN_QUERY, deleteBlogPost, publishBlogPost, unpublishBlogPost, duplicateBlogPost } from '@/src/sanity/lib/sanity'
import type { BlogPost } from '@/types/types'
import StatusBadge from '@/src/components/shared/StatusBadge'
import ConfirmDialog from '@/src/components/shared/ConfirmDialog'
import {
  Plus, Search, RefreshCw, Eye, Edit2, Trash2, Copy,
  Globe, EyeOff, FileText, ChevronDown, Image
} from 'lucide-react'
      ...
      {loading ? (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="card h-48 animate-pulse" style={{ background: 'rgba(255,255,255,0.02)' }} />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="card flex flex-col items-center justify-center py-16">
          <FileText className="w-12 h-12 mb-4" style={{ color: 'var(--text-faint)' }} />
                ...
                <tr key={post._id}>
                    ...
                    <div className="flex items-center gap-3">
                      {post.featuredImage ? (
                        <div className="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0">
                          <img
                            src={urlFor(post.featuredImage).width(80).height(80).url()}
                            alt={post.title}
                        ...
                        <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0"
                          ...
                          <Image className="w-4 h-4" style={{ color: 'var(--text-faint)' }} />
                        </div>
                      )}
                      <div className="min-w-0">
                        <p className="font-medium text-sm truncate max-w-xs" style={{ color: 'var(--text)' }}>
                          {post.title}
                        </p>
                  ...
                  <td>
                    <div className="flex flex-wrap gap-1">
                      {post.categories?.slice(0, 2).map(cat => (
                        <span key={cat._id} className="badge badge-draft text-xs" style={{ padding: '2px 6px' }}>
                          {cat.title}
                        </span>
                      ))}
                  ...
                  <td>
                    <div className="flex items-center gap-1">
                      <Link href={`/dashboard/blog/${post._id}`} className="btn-ghost p-1.5" title="Edit">
                      ...
                      <button
                        ...
                        style={{ color: post.status === 'published' ? '#FFC107' : '#28A745' }}
                      >
                      ...
                      <button
                        ...
                        className="btn-ghost p-1.5" title="Delete"
                        style={{ color: '#DC3545' }}
                      >
